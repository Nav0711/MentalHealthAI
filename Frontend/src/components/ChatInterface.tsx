...
const geminiResponse = async (message: string): Promise<string> => {
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ className }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await geminiResponse(input);
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle className="mental-text">Chat with your Mental Health Copilot</CardTitle>
      </CardHeader>
        ...
        <div className="h-[400px] overflow-y-auto p-4">
          ...
          {isLoading && (
            <div className="flex items-center gap-2 ml-12 mb-4">
              ...
              <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: themeColors.primary, animationDelay: "0.2s" }}></div>
              <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: themeColors.primary, animationDelay: "0.4s" }}></div>
            </div>
          )}
